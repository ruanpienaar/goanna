...
% % TODO: Move this into a function in a testing repo
...
%      fun cleanup/1,
%      [
%         {"goanna_api add nodes at startup",
%             ?_assert(unit_testing:try_test_fun(fun goanna_add_nodes_at_startup/0))
%         }
%      ]
%     }.

% goanna_add_nodes_at_startup() ->
%     {ok, Host} = inet:gethostname(),
%     Node = ?TEST_NODE(Host),
%     Cookie = cookie,
%     ok = application:load(goanna),
%     ok = application:set_env(goanna, nodes, [{Node, Cookie}]),
%     {ok,_} = goanna_api:start(),
%     ?assertEqual(
%         ok,
%         wait_for_node({Node,Cookie,tcpip_port}, 100, 25)
%     ),
...
%      [
%         {"goanna_api_add_node",
%             ?_assert(unit_testing:try_test_fun(fun goanna_api_add_node/0))},
%         {"goanna_api_add_node_validation",
%             ?_assert(unit_testing:try_test_fun(fun goanna_api_add_node_validation/0))},
%         {"remove_node",
%             ?_assert(unit_testing:try_test_fun(fun remove_node/0))},
%         {"remove_node_validation",
%             ?_assert(unit_testing:try_test_fun(fun remove_node_validation/0))},
%         {"update_default_trace_options",
%             ?_assert(unit_testing:try_test_fun(fun update_default_trace_options/0))},
%         {"update_default_trace_options_validation",
%             ?_assert(unit_testing:try_test_fun(fun update_default_trace_options_validation/0))},
%         {"set_data_retrival_method_validation",
%             ?_assert(unit_testing:try_test_fun(fun set_data_retrival_method_validation/0))},
%         {"set_data_retrival_method",
%             ?_assert(unit_testing:try_test_fun(fun set_data_retrival_method/0))},
%         {"trace",
%             ?_assert(unit_testing:try_test_fun(fun trace/0))},
%         {"trace_validation",
%             ?_assert(unit_testing:try_test_fun(fun trace_validation/0))},
%         {"stop_trace",
%             ?_assert(unit_testing:try_test_fun(fun stop_trace/0))},
%         {"reached_max_stop_trace time",
%            ?_assert(unit_testing:try_test_fun(fun reached_max_stop_trace_time/0))},
%         {"reached_max_stop_trace messages",
%            ?_assert(unit_testing:try_test_fun(fun reached_max_stop_trace_messages/0))},
%         {"list_active_traces",
%             ?_assert(unit_testing:try_test_fun(fun list_active_traces/0))}
%      ]
%     }.
...
%     {ok, Host} = inet:gethostname(),
%     {ok, _} = erlang_testing:start_distrib(list_to_atom("goanna_eunit_test@"++Host),
%         shortnames,
%         4000),
%     erlang_testing:slaves_setup([{Host, ?TEST_NODE_NAME}]).
...
%         goanna_api:nodes() == []
%     end, false),
%     ?assert(goanna_api:nodes() == []),
%     erlang_testing:stop_extra_applications(),
%     true = erlang_testing:cleanup_slaves(Slaves),
%     ok = erlang_testing:stop_distrib(),
%     timer:sleep(50).
...
%     ok = goanna_api:trace(goanna_test_module, function),
%     ok = goanna_api:trace(goanna_test_module, function2),
%     ok = goanna_api:trace(goanna_test_module, function2, 1),
